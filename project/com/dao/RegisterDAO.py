...
class RegisterDAO:
    ...
    def editProfile(self, registerVO):
        registerList = db.session.query(RegisterVO, LoginVO).join(LoginVO, RegisterVO.register_LoginId == LoginVO.loginId).filter(RegisterVO.register_LoginId == registerVO.register_LoginId).all()
        return registerList
